...
const ChartGauge: React.FC<GaugeProps> = ({ value, min, max, donvi, backgroundColor }) => {
  ...
  return (
    <div className="flex flex-col mx-auto items-center relative w-36 h-36 max-sm:w-30 max-sm:h-30">
      ...
      {/* Min - Max ở 2 đầu */}
      <div className="absolute bottom-[-18px] max-sm:bottom-[-12px] left-6 max-sm:left-2 text-lg font-medium max-sm:text-sm">Thấp</div>
      <div className="absolute bottom-[-18px] max-sm:bottom-[-12px] right-6 max-sm:right-2 text-lg font-medium max-sm:text-sm">Cao</div>
    </div>
  );
};
